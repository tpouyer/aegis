...
describe('Chat view renders with issue context', () => {
  ...
  it('MessageList renders existing messages with correct roles', () => {
    ...
    expect(
      screen.getByText('Looking at the issue, I recommend...'),
    ).toBeInTheDocument();
  });
  ...
  it('chat store adds messages and preserves order', () => {
    ...
    addMessage(
      'AEGIS-42',
      createMessage({ id: 'u1', role: 'user', content: 'First message' }),
    );
    addMessage(
      'AEGIS-42',
    ...
    addMessage(
      'AEGIS-42',
      createMessage({ id: 'u2', role: 'user', content: 'Follow-up' }),
    );
...
describe('Message input accepts text', () => {
  ...
  it('shows stop button during streaming', () => {
    ...
    render(
      <MessageInput onSend={onSend} onStop={onStop} isStreaming={true} />,
    );
  ...
  it('disables input when disabled prop is true', () => {
    ...
    render(
      <MessageInput onSend={onSend} isStreaming={false} disabled={true} />,
    );
...
describe('System prompt includes issue context', () => {
  ...
  it('includes issue description when provided', () => {
    ...
    expect(prompt).toContain(
      'Users are stuck in an infinite redirect after SSO login.',
    );
  });
  ...
  it('includes acceptance criteria when provided', () => {
    const prompt = buildSystemPrompt({
      ...
      issueSummary: 'Fix login redirect loop',
      acceptanceCriteria:
        '- Login redirect should complete within 3 seconds\n- No infinite loops',
      supportsToolUse: false,
    });
