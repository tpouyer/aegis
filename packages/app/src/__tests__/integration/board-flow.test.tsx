...
describe('Board column and card population', () => {
  it('derives columns from board config and assigns issues correctly', () => {
    const columns: BoardColumn[] = mockBoardConfig.columnConfig.columns.map((col) => {
      const statusIds = new Set(col.statuses.map((s) => s.id))
      const columnIssues = issues.filter((issue) => statusIds.has(issue.fields.status.id))
      return {
        name: col.name,
  ...
  it('handles optimistic updates by reassigning issues to target columns', () => {
    ...
    expect(columns[1].issues.map((i) => i.key).sort()).toEqual(['AEGIS-1', 'AEGIS-2'])
  })
})
...
describe('FilterBar integration with board store', () => {
  ...
  it('text filter updates the store and can filter issues client-side', async () => {
    ...
    const filteredIssues = issues.filter(
      (issue) => issue.key.toLowerCase().includes('auth') || issue.fields.summary.toLowerCase().includes('auth'),
    )
    ...
    expect(filteredIssues.map((i) => i.key).sort()).toEqual(['AEGIS-1', 'AEGIS-3'])
