...
export function TransitionModal({
  ...
  const requiredFields = useMemo(
    () => fields.filter((f) => f.meta.required),
    [fields],
  );
    ...
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) handleCancel(); }}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This transition requires additional information before it can be
            completed.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4 py-2">
          {fields.length === 0 && (
            <p className="text-sm text-muted-foreground">
              No additional fields are needed for this transition.
            </p>
          )}
          ...
          {fields.map(({ fieldId, meta }) => (
            <div key={fieldId} className="flex flex-col gap-1.5">
              <label
                htmlFor={`transition-field-${fieldId}`}
