 ...
 */
export async function handleGoogleCallback(params: URLSearchParams, config: GoogleOAuthConfig): Promise<TokenSet> {
  ...
  if (data.error) {
    throw new Error(`Google token exchange error: ${data.error_description || data.error}`)
  }
