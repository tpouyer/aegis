...
export async function discoverOIDCConfig(
  ...
  if (!doc.authorization_endpoint || !doc.token_endpoint) {
    throw new Error(
      `OIDC discovery document for ${issuerUrl} missing required endpoints`,
    );
  }
 ...
 */
export async function handleRedHatCallback(
  ...
  if (!response.ok) {
    throw new Error(
      `Red Hat SSO token exchange failed: ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (data.error) {
    throw new Error(
      `Red Hat SSO token exchange error: ${data.error_description || data.error}`,
    );
  }
