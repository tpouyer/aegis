...
export class JiraClient {
  ...
  async getBoardConfig(boardId: number): Promise<JiraBoardConfig> {
    return this.request<JiraBoardConfig>(this.agileUrl(`/board/${boardId}/configuration`))
  }
   ...
   */
  async getIssuesForBoard(boardId: number, jql?: string, startAt = 0, maxResults = 100): Promise<JiraSearchResponse> {
    const params = new URLSearchParams({
      startAt: String(startAt),
      maxResults: String(maxResults),
    })
    ...
    return this.request<JiraSearchResponse>(this.agileUrl(`/board/${boardId}/issue?${params}`))
  }

  /** Get a single issue with all fields. */
  async getIssue(issueKey: string): Promise<JiraIssue> {
    return this.request<JiraIssue>(this.apiUrl(`/api/3/issue/${encodeURIComponent(issueKey)}`))
  }
   ...
   */
  async searchIssues(jql: string, fields?: string[], maxResults = 100, startAt = 0): Promise<JiraSearchResponse> {
    return this.request<JiraSearchResponse>(this.apiUrl('/api/3/search'), {
      method: 'POST',
      body: JSON.stringify({
        jql,
        fields: fields ?? [
   ...
   */
  async doTransition(issueKey: string, transitionId: string, fields?: Record<string, unknown>): Promise<void> {
    ...
    await this.request<void>(this.apiUrl(`/api/3/issue/${encodeURIComponent(issueKey)}/transitions`), {
      method: 'POST',
  ...
  async updateIssue(issueKey: string, fields: Record<string, unknown>): Promise<void> {
    await this.request<void>(this.apiUrl(`/api/3/issue/${encodeURIComponent(issueKey)}`), {
      method: 'PUT',
  ...
  async addComment(issueKey: string, body: unknown): Promise<void> {
    await this.request<void>(this.apiUrl(`/api/3/issue/${encodeURIComponent(issueKey)}/comment`), {
      method: 'POST',
      body: JSON.stringify({ body }),
    })
  }
  ...
  async getCurrentUser(): Promise<JiraUser> {
    return this.request<JiraUser>(this.apiUrl('/api/3/myself'))
  }
}
...
export function getJiraClient(): JiraClient {
  if (!clientInstance) {
    throw new Error('JiraClient not initialized. Call initJiraClient(config) first.')
  }
