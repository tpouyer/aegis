...
describe('resilientFetch', () => {
  ...
  it('retries on 500 with exponential backoff', async () => {
    ...
    mockFetch.mockResolvedValueOnce(fail).mockResolvedValueOnce(fail).mockResolvedValueOnce(success)
  ...
  it.each([400, 401, 403, 404])('does not retry on %i', async (status) => {
  ...
  it('GET deduplication: concurrent identical GETs share one fetch', async () => {
    ...
    const [r1, r2, r3] = await Promise.all([resilientFetch(url), resilientFetch(url), resilientFetch(url)])
  ...
  it('does not deduplicate POST requests', async () => {
    ...
    await Promise.all([resilientFetch(url, options), resilientFetch(url, options)])
  ...
  it('AbortSignal cancels retries', async () => {
    ...
    const fetchPromise = resilientFetch('https://api.example.com/data', { signal: controller.signal })
  ...
  it('max retries exceeded on network errors throws', async () => {
    ...
    await expect(
      resilientFetch('https://api.example.com/data', { method: 'POST' }, { maxRetries: 2, baseDelay: 1, maxDelay: 5 }),
