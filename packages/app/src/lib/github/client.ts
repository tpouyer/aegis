...
import type { FileContent, GitCommit, GitRef, PullRequest, RepoInfo, TreeEntry } from './types'
...
export class GitHubClient {
  ...
  async getTree(owner: string, repo: string, sha: string, recursive?: boolean): Promise<TreeEntry[]> {
    const qs = recursive ? '?recursive=1' : ''
    const data = await this.request<{ tree: TreeEntry[] }>(`/repos/${owner}/${repo}/git/trees/${sha}${qs}`)
    return data.tree
  }

  async getRef(owner: string, repo: string, ref: string): Promise<GitRef> {
    const data = await this.request<{ ref: string; object: { sha: string } }>(`/repos/${owner}/${repo}/git/ref/${ref}`)
  ...
  async getBlob(owner: string, repo: string, sha: string): Promise<string> {
    const data = await this.request<{ content: string; encoding: string }>(`/repos/${owner}/${repo}/git/blobs/${sha}`)
  ...
  async createBranch(owner: string, repo: string, branchName: string, fromSha: string): Promise<GitRef> {
    const data = await this.request<{ ref: string; object: { sha: string } }>(`/repos/${owner}/${repo}/git/refs`, {
      method: 'POST',
      body: JSON.stringify({
   ...
   */
  async createBlob(owner: string, repo: string, content: string, encoding: string = 'utf-8'): Promise<string> {
    const data = await this.request<{ sha: string }>(`/repos/${owner}/${repo}/git/blobs`, {
      method: 'POST',
  ...
  async createTree(
    ...
    const data = await this.request<{ sha: string }>(`/repos/${owner}/${repo}/git/trees`, {
      method: 'POST',
  ...
  async createCommit(owner: string, repo: string, message: string, tree: string, parents: string[]): Promise<string> {
    const data = await this.request<{ sha: string }>(`/repos/${owner}/${repo}/git/commits`, {
      method: 'POST',
