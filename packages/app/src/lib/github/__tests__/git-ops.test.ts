...
describe('atomicCommit', () => {
  ...
  it('calls createBlob for each added/modified file', async () => {
    ...
    await atomicCommit(mockGithub, 'org', 'repo', 'feature/test', 'base-sha', 'base-tree-sha', changes, 'test commit')
  ...
  it('updates ref to new commit SHA', async () => {
    const changes: FileChange[] = [{ path: 'src/a.ts', status: 'added', currentContent: 'code', repo: 'org/repo' }]
    ...
    expect(mockGithub.updateRef).toHaveBeenCalledWith('org', 'repo', 'heads/feature/test', 'commit-sha-new')
  })
  ...
  it('chains operations in correct order: blobs -> tree -> commit -> ref', async () => {
    ...
    const changes: FileChange[] = [{ path: 'a.ts', status: 'added', currentContent: 'x', repo: 'org/repo' }]

    await atomicCommit(mockGithub, 'org', 'repo', 'feat', 'base', 'base-tree', changes, 'msg')
