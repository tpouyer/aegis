...
describe('GitHubClient', () => {
  ...
  describe('getTree', () => {
    it('builds correct URL with recursive flag', async () => {
      ...
      expect(calledUrl).toBe(
        'https://api.github.com/repos/org/repo/git/trees/sha123?recursive=1',
      );
    });
    ...
    it('builds correct URL without recursive flag', async () => {
      ...
      const calledUrl = mockFetch.mock.calls[0][0] as string;
      expect(calledUrl).toBe(
        'https://api.github.com/repos/org/repo/git/trees/sha123',
      );
    });
  });
  ...
  describe('error handling', () => {
    it('throws on non-2xx response', async () => {
      ...
      await expect(
        client.getTree('org', 'repo', 'bad-sha'),
